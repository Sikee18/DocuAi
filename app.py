...
# 4. Expose the precompiled static payload directly via FastAPI Fallback router!
static_dir = os.path.join(os.getcwd(), ".web", "_static")
if os.path.exists(static_dir):
    # This efficiently serves all our prebuilt HTML/JS statically safely.
    app.api.mount("/", StaticFiles(directory=static_dir, html=True), name="frontend")
    print(f"Serving UI Catch-All from {static_dir}")

# We will run this file using `uvicorn app:app` !!
# By using exactly `app:app`, `rx.App.__call__` natively executes its own internal highly-robust socket.io proxy for `/_event`, and effortlessly falls back to `app.api` for the static site!
